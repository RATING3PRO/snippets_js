...
## Telegram API 反向代理
在中国大陆开发 Telegram Bot 时，由于 `api.telegram.org` 无法被直接访问，可以通过这个脚本将其代理至 Cloudflare 域名。

**特点:**
* 简单直接请求转发，代理包括 Webhook 设定、发送消息等各类 API。
* 可直接将你被墙服务器上的 Bot 请求修改至绑定的 Cloudflare 域名。
